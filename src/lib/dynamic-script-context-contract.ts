export function buildDynamicScriptContextTypeContract() {
  return {
    ...
    repositories: {
      ...
      guarantee: 'find/create/update and createMany/updateMany return collection-shaped results with data arrays. createMany/updateMany/deleteMany return count. Batch methods are only supported for plain generic tables: they reject metadata/schema routes and tables with custom normalization or lifecycle hooks. updateMany applies one data object to every id and rejects relation payloads. Never guard result.data with Array.isArray. Use result.data[0] after a proven match, or result.data?.[0] ?? null when zero rows is valid.',
    ...
    services: {
      ...
      '@RES': 'HTTP handler only: stream(readable, options?): Promise<void>. The readable may be a package-backed readable, the replay stream returned by $ctx.$streams.preflight, or another approved server-side readable. Direct @RES.stream(upstream.body, ...) starts the public response before consuming the body; use preflight first when the application must retry an upstream that produces no bytes. Options may include statusCode, mimetype, filename, headers, observer?: (chunkText: string, kind: "chunk" | "end" | "error") => unknown | Promise<unknown>, and transform?: (chunkText: string, kind: "chunk" | "end") => string | null | undefined | Promise<string | null | undefined>. transform receives UTF-8-decoded fragments before relay: one decoder spans the full stream, so a Unicode code point split across byte chunks is intact. A fragment may still end inside an SSE line or event, so SSE transforms keep a per-request buffer and parse only complete blank-line-delimited events. Return a string to replace output, null to suppress it, or undefined to preserve it; transform failures fail the stream. observer remains best-effort and is not a pre-response readiness primitive. Await @RES.stream(...) and do not return another payload afterward. Direct relay remains bounded by the selected method timeout; client abort/response close cancels the task.',
