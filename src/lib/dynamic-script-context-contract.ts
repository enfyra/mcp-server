export function buildDynamicScriptContextTypeContract() {
  return {
    ...
    services: {
      ...
      '@RES': 'HTTP handler only: stream(readable, options?): Promise<void>. The readable must be a server-side Node readable or approved Web ReadableStream exposed by a server package bridge. Options may include statusCode, mimetype, filename, headers, and observer?: (chunkText: string, kind: "chunk" | "end" | "error") => unknown | Promise<unknown>. The observer sees arbitrary decoded fragments, then one empty end/error notification; it is best-effort and awaited before each chunk is relayed. It pipes directly to the client, so await @RES.stream(...) and do not return another payload afterward. The selected method timeout remains the single deadline for observer work and stream relay; client abort/response close cancels the task. This is the streaming response boundary; use @HELPERS.$fetch only for buffered json/text/ArrayBuffer responses.',
