...
export function buildMcpServerInstructions(apiBaseUrl, options: McpInstructionOptions = {}) {
  ...
  return [
    ...
    '- Load examples only when needed with `get_enfyra_examples`. For OAuth use `get_enfyra_examples({ category: "oauth-setup" })`. For extension UI, call `get_extension_theme_contract` before writing or reviewing UI, use `build_extension_ui` after the extension acknowledgement for high-contract components such as drawers, modals, page shell headers/actions, permission gates, empty states, resource lists, FormEditor, Widget, menu/account panel registries, tabs, upload modals, and review, and call `get_theme_class_reference` only when exact classes are needed.',
