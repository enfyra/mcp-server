...
export function buildMcpServerInstructions(apiBaseUrl, options: McpInstructionOptions = {}) {
  ...
  return [
    ...
    '- Load examples only when needed with `get_enfyra_examples`. For OAuth use `get_enfyra_examples({ category: "oauth-setup" })`. For extension UI, call `get_extension_theme_contract` before writing or reviewing UI and `get_theme_class_reference` only when exact classes are needed.',
    '- Prefer the most specific business operation tool over raw metadata CRUD. For writes, call `get_enfyra_required_knowledge` and pass `globalRulesAckKey`; dynamic server code also needs `knowledgeAckKey`; extension code also needs `extensionKnowledgeAckKey`.',
