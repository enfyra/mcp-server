...
export const TOOL_WORKFLOWS = [
  {
    ...
    useWhen: [
      ...
      'Decomposing a large or mixed-responsibility route handler into maintainable runtime layers.',
    ],
    keywords: ['endpoint', 'api', 'route handler', 'handler', 'webhook', 'custom route', 'rest path', 'third-party api', 'third party api', 'external integration', 'large handler', 'split handler', 'decompose handler'],
    firstTools: ['get_enfyra_required_knowledge', 'discover_script_contexts', 'resolve_route_context'],
    inspectTools: ['resolve_route_context', 'inspect_route', 'get_all_routes', 'list_methods', 'trace_metadata_usage'],
    ...
    nextStepTemplate: [
      ...
      'Assess source cohesion before saving. Keep endpoint-specific business orchestration and small local helpers in the handler; decompose shared gating/normalization into named pre-hooks, response or best-effort work into named post-hooks, and durable or retryable work into flows.',
  ...
  },
  {
    ...
    useWhen: [
      ...
      'Splitting a large page or global extension into focused widget/global extension records.',
    ],
    keywords: ['extension', 'menu', 'account panel', 'notification', 'chip', 'badge', 'sidebar', 'shell', 'page ui', 'widget', 'large extension', 'split extension', 'decompose page'],
    ...
    nextStepTemplate: [
      ...
      'Assess SFC cohesion before saving. Keep shell, routing, and top-level coordination in the page; split independent feature sections, repeated panels, or unrelated API/state domains into focused widget extensions composed by numeric id with explicit props/events. Split unrelated shell-wide behaviors into separate global extensions.',
  ...
  },
  {
    ...
    keywords: ['sourcecode', 'script', 'hook', 'pre hook', 'post hook', 'hook layering', 'god hook', 'split handler', 'large script', 'decompose script', 'compiledcode', 'macro', 'repos', 'bootstrap'],
    ...
    nextStepTemplate: [
      ...
      'Assess cohesion before saving: split mixed-responsibility code through the native boundary for that surface, while preserving endpoint-specific business logic and any transaction that truly must remain atomic.',
      ...
      'For flows use multiple named steps with one business operation each; for websocket code keep connection lifecycle in the gateway and one event contract per event record; keep OAuth provisioning and bootstrap sources focused on their own lifecycle contracts.',
  ...
  },
  {
    ...
    firstTools: ['get_permission_profile', 'resolve_route_context', 'audit_route_access'],
    inspectTools: ['get_permission_profile', 'resolve_route_context', 'inspect_route', 'audit_route_access'],
    knowledgeTools: ['get_enfyra_required_knowledge'],
  ...
  },
  {
    ...
    nextStepTemplate: [
      ...
      'Keep one business operation per named step. Split mixed-responsibility script logic into fixed/script/condition steps or a triggered child flow, but keep repository operations together when they genuinely require one atomic transaction boundary.',
  ...
  },
  {
    ...
    nextStepTemplate: [
      ...
      'Ensure the gateway first and keep its source focused on connection lifecycle/authentication/setup. Put each message behavior in one event record, split unrelated event names into separate records, and use flows for durable or retryable side effects.',
  ...
  },
  {
    ...
    nextStepTemplate: [
      ...
      'Remember GraphQL table data requires authenticated JWT or PAT credentials even when REST is public.',
    ],
    recommendedScope: 'schema',
  },
  {
  ...
  },
  {
    ...
    useWhen: [
      'Reading database-backed system errors and user logs.',
      'Searching logs for route, flow, websocket, or dynamic script failures.',
      'Inspecting isolated-executor worker crashes and their active script diagnostics.',
      'Running admin tests for supported runtime surfaces.',
      'Sending pending MCP tool errors to Enfyra telemetry immediately.',
    ],
    keywords: ['log', 'debug', 'error', 'trace', 'tail', 'diagnostic', 'worker crash', 'executor crash', 'runner crash', 'test runner', 'report MCP error', 'send MCP error'],
    firstTools: ['search_system_errors', 'search_user_logs'],
    inspectTools: ['search_system_errors', 'search_user_logs'],
    knowledgeTools: [],
    ...
    avoidTools: [
      {
        ...
        useInstead: 'search_system_errors or search_user_logs',
    ...
    exampleCategories: [],
    nextStepTemplate: [
      'Search a bounded time window in the database log tables.',
      'Use search_system_errors first for an isolated-executor worker or runner exit.',
  ...
  },
  {
    ...
    useWhen: [
      ...
      'Debugging direct MCP PAT authentication or current MCP user permissions.',
      'Checking whether non-root tokens can call admin helper routes.',
    ],
    keywords: ['auth', 'token', 'current user', 'permission profile', 'target', 'api base', '403', 'PAT header'],
