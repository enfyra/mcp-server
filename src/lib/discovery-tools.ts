...
import { buildGraphqlUrls } from './mcp-instructions.js';
import {
    ...
    async ({ tableName }) => {
      ...
      const payload = {
        ...
        queryParams: {
          ...
          aggregate: 'Grouped analytics are available only through dynamic repository aggregate({ filter, dimensions, measures, sort, page, limit }); query_table does not accept aggregate.',
        },
        ...
        aggregate: {
          shape: '{ filter?, dimensions: [{ field, bucket?, timezone? }], measures: { output: { count|countDistinct|sum|avg|min|max: field } }, sort?, page?, limit? }',
          scalarOperations: 'Measures are named computed outputs over scalar fields. sum/avg/min/max require numeric live fields; count/countDistinct accept any scalar field.',
          dimensions: 'Dimensions are scalar fields or date buckets using hour/day/week/month/year and an IANA timezone.',
          result: 'Aggregate returns computed rows under data and never returns raw records. Pagination applies to grouped rows.',
          example: 'aggregate({ filter: { status: { _eq: "running" } }, dimensions: [{ field: "createdAt", bucket: "day", timezone: "Asia/Ho_Chi_Minh" }], measures: { requests: { count: "id" }, totalTokens: { sum: "totalTokens" } }, sort: [{ field: "createdAt", direction: "asc" }] })',
        },
      ...
      const payload = {
        ...
        contexts: {
          preHook: {
            ...
            projectionContract: 'For canonical table reads, preserve client-controlled query shape. Do not override @QUERY.fields, @QUERY.deep, @QUERY.sort, @QUERY.limit, @QUERY.page, @QUERY.meta, or debugMode. RLS should only merge security constraints into @QUERY.filter. Grouped analytics use a separate repository.aggregate call.',
