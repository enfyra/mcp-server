...
export function buildMcpServerInstructions(apiBaseUrl) {
  ...
  return [
    ...
    '- **Execution history** (`flow_execution_definition`): `flow` → flow id, `status`, `triggerPayload`, `context` (full data chain), `completedSteps`, `currentStep`, `error`, `startedAt`, `completedAt`, `duration`. Query separately — NOT nested under flow_definition.',
    ...
    '- **Workflow**: Create flow → `create_record` on `flow_definition`. Add steps → `create_record` on `flow_step_definition` with `flow: {id}`. Trigger manually via `POST /admin/flow/trigger/{flowId}` or via schedule/event/webhook.',
