...
const GLOBAL_RULES_SECTIONS = [
  ...
  },
  {
    ...
    rules: [
      ...
      'Only enfyra is a public tool. Discover the exact internal operation name with action=discover, then use action=execute with its returned schema. Guided reads and writes retain their own safety gates; low-level escape hatches are unavailable.',
  ...
  },
  {
    ...
    rules: [
      ...
      'A global guard can carve out exceptions with excludePaths/excludeRouteIds on ensure_guard. Both require isGlobal=true and are rejected on route-scoped or type=graphql guards. Exclusion matches the declared metadata route path exactly, so exclude /assets/:id rather than a concrete URL. Use it for "protect everything except these routes"; use a route-specific guard when only one route needs a different policy.',
  ...
  },
  {
    ...
    rules: [
      ...
      'Use plural mutation tools for ordinary writes: create_tables/update_tables/delete_tables, create_columns/update_columns/delete_columns, create_relations/update_relation_constraints/delete_relations, and create_records/update_records/delete_records. Pass native JSON arrays; use one item in the array for a single mutation. create_inverse_relation is intentionally singular because it acts on one exact owning relation after a reasoning gate.',
      ...
      'create_tables and create_relations create owning relations only and reject inversePropertyName, mappedBy, and one-to-many inputs. After creation, evaluate inverseDecision. Keep the relation one-way by default; call create_inverse_relation only when a concrete named response, UI, deep query, aggregate, or parent-to-child workflow needs target-to-source traversal, and state that consumer in the tool input.',
      'ESV persists an inverse as a separate relation on the target with mappedBy pointing to the owning relation. The inverse type is derived from the owner: many-to-one becomes one-to-many, one-to-one stays one-to-one, and many-to-many stays many-to-many. Never patch inversePropertyName onto an existing owning relation to add an inverse.',
      ...
      'For reads, query_table accepts native object filter/deep values. Deep keys are relation names; query_table auto-adds missing top-level deep relation keys to fields so nested records can appear. Explicit dotted fields and fields inside deep are recursively checked against each relation target metadata before the REST request. Inside deep, use fields/filter/sort/limit/page/deep; never use _fields. Grouped analytics use repository.aggregate({ filter, dimensions, measures, sort, page, limit }) in dynamic scripts.',
      ...
      'query_table always requires limit or all=true. Use meta=filterCount/totalCount or count_records for counts. Grouped analytics are not query_table options.',
      'Repository aggregate queries use aggregate({ filter, dimensions?, measures, sort, page, limit }). With dimensions omitted or empty, it returns one scalar summary row such as a filtered order count; with dimensions present, it returns grouped rows. Dimensions are scalar fields or date buckets (hour/day/week/month/year with an IANA timezone); measures are named count, countDistinct, sum, avg, min, or max operations over scalar fields. Results contain computed rows only under data and never raw records. Do not aggregate encrypted/unpublished fields or fields denied by field permissions.',
...
const DYNAMIC_CODE_SECTIONS = [
  ...
  },
  {
    ...
    rules: [
      ...
      'For canonical table reads and shared RLS, merge security filters into @QUERY.filter and preserve @QUERY.fields, @QUERY.deep, @QUERY.sort, @QUERY.limit, @QUERY.page, @QUERY.meta, and debugMode. Grouped analytics use a separate repository.aggregate call.',
    ],
  },
  {
    ...
    rules: [
      ...
      'Keep OAuth lifecycle and bootstrap scripts focused on their own lifecycle contract. Use local helper functions for cohesive transformations, and split independent work into supported records or flows only when runtime ordering and atomicity remain explicit; do not invent static imports or unsupported shared modules.',
  ...
  },
  {
    ...
    rules: [
      ...
      'Prefer macros such as @BODY, @QUERY, @PARAMS, @USER, @REQ, @RES, @REPOS, @HELPERS, @STORAGE, @SOCKET, @TRANSACTION, and @THROW* when available. Use await @TRANSACTION.run(async () => { ... }) for all-or-fail repository mutations; fetch, storage, cache, socket, and flow-trigger effects do not roll back.',
      ...
      'Call build_dynamic_repository_usage for list, find-one, create/createMany, update/updateMany, or delete/deleteMany code instead of composing secure/trusted repository syntax and result shapes from memory. Batch methods use one generic-table lifecycle/reload/event boundary: createMany takes data rows, updateMany takes ids plus one shared data object, and deleteMany takes ids. They reject schema/custom-lifecycle tables; updateMany also rejects relation payloads.',
      'An enfyra_oauth_config sourceCode script runs once per successful provider callback inside the server-owned database transaction after the user and OAuth account are resolved and before session creation. @USER is the persisted resolved user. @DATA.oauth.event is user_created or login; @DATA.oauth.profile contains providerUserId, email, emailVerified, name, givenName, familyName, username, avatarUrl, profileUrl, and locale; provider-specific extras are in claims, while accessToken and token metadata are ephemeral. Perform repository mutations and do not return a value. Failure rolls back the OAuth database unit of work and returns no Enfyra tokens. An unlinked provider identity with the same email as an existing user returns a conflict and is never auto-linked.',
      ...
      'Package-backed readable contract: package response bodies are single-consumer AsyncIterable<Uint8Array> values. Pull them with for await...of, pass them once to $ctx.$streams.preflight/readBytes/readText, or relay them once through @RES.stream. Reusing a body or issuing concurrent next() calls fails deterministically; make a new upstream request for each retry.',
      'Pre-response streaming contract: use await $ctx.$streams.preflight(upstream.body, { timeoutMs }) when the application must observe the first non-empty raw chunk before committing the public response. It returns { stream, firstChunk }; relay only result.stream so the first chunk appears exactly once. Any non-empty raw bytes, including reasoning-only SSE data, count as activity. The helper does not parse SSE or wait for visible text.',
      'A preflight timeout is stream-scoped: it closes only the target upstream and throws ERR_PACKAGE_STREAM_TIMEOUT, so application code may catch it and issue a new request in the same executor task. Empty-before-first-byte throws ERR_PACKAGE_STREAM_EMPTY. Per-stream timeoutMs never extends the enclosing handler or flow-step deadline; task timeout/client cancellation still cleans up all task-owned streams.',
      'Response-independent package consumption uses $ctx.$streams.readBytes/readText(readable, { timeoutMs, maxBytes }). The helpers do not start @RES, default maxBytes to 8 MiB, count raw bytes rather than string length, return an empty value for an empty stream, reject source errors without partial success, and decode readText incrementally across split UTF-8 code points.',
      'HTTP streaming response contract: @RES.stream(readable, options?) is the commit boundary for a custom HTTP handler. Direct @RES.stream(upstream.body, ...) starts the public response before the package body is consumed; preflight first when a no-byte upstream must remain retryable. The handler must await @RES.stream(...) and must not return a second JSON payload afterward.',
      ...
      'observer and transform run only after @RES.stream starts response relay. They are not preflight/readiness primitives. Set the method timeout high enough for the complete upstream call and stream; any shorter preflight/read timeout is capped by that enclosing deadline.',
      ...
      'Package installation alone is not enough to promise streaming. Verify package request -> preflight first raw chunk -> replay stream -> @RES.stream -> client response for streaming clients, and package request -> bounded readText/readBytes -> validate -> buffered response for buffered clients.',
...
const SCOPED_PURPOSES: Record<KnowledgeScope, string> = {
  ...
  'dynamic-code': 'Read this before writing or mutating dynamic server code (handlers, hooks, flow steps, websocket events, OAuth lifecycle scripts, bootstrap scripts) through MCP.',
